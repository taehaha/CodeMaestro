...
const initialGreeting: Message = {
  sender: "bot",
  text: "안녕하신가 젊은이! 나는 리치. 전설적인 프로그래머의 클론으로써, C 언어와 프로그래밍에 관한 깊은 지식을 가지고 있다네. 자네와 기술적 이야기를 나눌 수 있어서 기쁘군. 궁금한 점이 있으면 언제든 물어보게!",
  type: "text",
};
    ...
    try {
        ...
        const accumulatedReviewMessages = [
          {
            role: "system",
            content:  `
            당신은 리치입니다. 사용자의 실력 수준은 "${skillLevel}"입니다. 사용자의 실력에 맞는 대답을 해줍니다. 밑 내용이 사용자 실력에 맞지 않다면 생략도 가능합니다. 전설적인 프로그래머이자 "C 언어"와 "유닉스 운영체제"의 창시자의 클론으로, 프로그래밍의 기본부터 복잡한 시스템 설계까지 모든 영역에서 뛰어난 전문가로 알려져 있습니다. 
            **당신의 역할:**
      ...
      if (isEditorRequest) {
        ...
        if (lastBotMessage) {
          ...
          if (codeMatch && codeMatch[1]) {
            ...
            setMessages(prev => [
              ...prev,
              { sender: "bot", text: "코드가 에디터에 반영되었네! 확인해보게나!", type: "text" },
            ]);
          } else {
            setMessages(prev => [
              ...prev,
        ...
        messages: [
          {
            role: "system",
            content:
              "다음 질문이 리치 칭찬, 인사, 리치소개, 프로그래밍이나 컴퓨터 과학과 관련 있는지 여부를 확인하세요. " +
      ...
      if (relevance.toLowerCase() === "no") {
        setMessages(prev => [
          ...
          {
            sender: "bot",
            text: "허허허 아쉽지만 나는 프로그래밍 관련이나 컴퓨터 관련 지식만 답해줄 수 있다네 젊은이.",
            type: "text",
          },
        ]);
        setIsTyping(false);
        ...
        {messages.map((message, index) => (
          ...
          >
            ...
            <div className="message-bubble">
              {message.sender === "bot" && (
                <span className="sender-label">리치</span>
              )}
